...
def clear_folder_pathlib(folder_path):
    """Очистка папки с использованием pathlib"""
...
for i in range(0, len(panel_images), 6):
    ...
    strip_image = create_strip(batch)
    strip_path = f"output/strip-{i//6 + 1}.png"
    strip_image.save(strip_path)
    pdf_pages.append(strip_image)  # сохраняем изображение для PDF

# Создаем PDF из всех изображений
if pdf_pages:
    pdf_pages[0].save(
        "output/comics.pdf",
        "PDF",
